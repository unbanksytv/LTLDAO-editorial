...
const WalletConnect = () => {
  const { authenticate, isWeb3Enabled, enableWeb3, isAuthenticated } =
    useMoralis();

  const signIn = () => {
    authenticate({
      provider: "walletconnect",
      signingMessage: "LearnDAO Authentication",
    });
  };
  return (
    <>
      <li>
        <button
          ...
          className="w-full flex items-center p-3 text-base font-bold text-gray-900 bg-gray-50 rounded-lg hover:bg-gray-100 group hover:shadow dark:bg-gray-600 dark:hover:bg-gray-500 dark:text-white"
        >
          <svg
            className="h-5"
            viewBox="0 0 512 512"
